 <div class="loginform" >
        <h4>Add user Details</h4>
        <form action="" id="validation">
              
            <label for="" class="label">Username</label>
            <input class="userinput"type="text" placeholder="username" id="usernames">
            <span class="tag" id="user" style="color: white; font-weight: bold;"></span>
            <label for=""class="label" >Password  </label>
            <input  class="userinput"type="password" placeholder="password" id="password">
            <span  class="tag" style="top:180px;color: white; font-weight: bold;" id="pass"> </span>
            <label for="" class="label">Confirm Password </label>
            <input class="userinput"type="password" placeholder="confirm password" id="confirmpassword">
            <span  class="tag" style="top:260px; color: white; font-weight: bold;" id="confirm"> </span>
            <button style="left: 200px; top:295px;" class="confirm">Submit</button>
            <button style="left: 310px; top:295px; background-color: #1b86b0;" class="close">Cancel</button>
        </form>

     </div>   